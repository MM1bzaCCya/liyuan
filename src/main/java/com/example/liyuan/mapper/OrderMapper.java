...
public interface OrderMapper {
    @Insert("INSERT INTO orders (request_id, grab_id, user_id, singer_id, song_name, original_singer, status, queue_number, sing_time, complete_time, create_time, update_time) " +
            "VALUES (#{requestId}, #{grabId}, #{userId}, #{singerId}, #{songName}, #{originalSinger}, #{status}, #{queueNumber}, #{singTime}, #{completeTime}, #{createTime}, #{updateTime})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(Order order);

    @Update("UPDATE orders SET status=#{status}, queue_number=#{queueNumber}, sing_time=#{singTime}, complete_time=#{completeTime}, update_time=#{updateTime} WHERE id=#{id}")
